...
final class WindHttpSocket extends AbstractWindHttp
{
    ...
    protected function createHttpHandler()
    {
        ...
        $this->port = isset($url['port']) ? $url['port'] : (isset($url['scheme']) ? ($url['scheme'] == 'https' ? 443 : 80) : 80);
    ...
    {
        return fwrite($this->httpHandler, ($value ? $name.': '.$value : $name));
    }
    ...
    public function send($method = 'GET', $options = array())
    {
        ...
        $this->setHeader($this->host.($this->port == 80 || $this->port == 443 ? '' : ':'.$this->port), 'Host');
