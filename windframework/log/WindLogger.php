...
class WindLogger extends WindModule
{
     ...
     * @param bool   $flush 是否将日志输出到文件,为true的时候将写入文件,默认为false
     */
     ...
     * @param string $type  日志的类型,默认为wind.system
     * @param bool   $flush 是否将日志输出到文件,为true则将会写入文件,默认为false
     */
    ...
    public function flush()
    {
        ...
        if ($this->_writeType & 1) {
            foreach ($_logLevels as $key => $value) {
                ...
                WindFile::write($fileName, implode('', $value), 'a');
            }
        }
        ...
        if ($fileName = $this->_getFileName()) {
            WindFile::write($fileName, implode('', $_l), 'a');
        }
        $this->_logs = array();
        $this->_logCount = 0;
     ...
     * 返回内存使用量
     *
     * @param  bool $peak 是否是内存峰值,默认为true
     * @return int
     */
