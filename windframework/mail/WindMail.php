...
class WindMail
{
    ...
    public function createHeader()
    {
        ...
        if (!isset($this->mailHeader[self::CONTENTENCODE])) {
    ...
    private function _getEncoder($encode)
    {
        ...
        switch ($encode) {
            case self::ENCODE_QP:
                $mailEncoder = 'WindMailQp';
                break;
            case self::ENCODE_BASE64:
                $mailEncoder = 'WindMailBase64';
                break;
            case self::ENCODE_7BIT:
            case self::ENCODE_8BIT:
            default:
                $mailEncoder = 'WindMailBinary';
                break;
        }
        if (!class_exists($mailEncoder)) {
